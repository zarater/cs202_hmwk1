...
info :: info(): source(NULL), day(NULL), e(NULL), m(NULL), l(NULL), escheduled_event(NULL), mscheduled_event(NULL), lscheduled_event(NULL)
{
...
char* info :: initdata(char source[], const char todet[])
{
...
//info*& node :: sethead(info* newinfo)
//{
//	data = newinfo;
//	return data;
//}
...
{
	node *temp = new node;
	temp -> sethead() -> initdata(a_day, "day");
	temp -> sethead() -> initdata(a_e, "e");
	temp -> sethead() -> initdata(a_m, "m");
	temp -> sethead() -> initdata(a_l, "l");
	temp -> sethead() -> initdata(a_escheduled_event,"esched");
	temp -> sethead() -> initdata(a_mscheduled_event, "msched");
	temp -> sethead() -> initdata(a_lscheduled_event, "lsched");
	into_aofLLL(temp);
    into_cll(temp);
		++x;
        r=x;
...
{
	//int i will be allocated when x iterates in set function i = 0 - 6
	node * temp = filled_node;
	if(calendar[r]->sethead() == NULL)
	{
		calendar[r]->sethead() = temp ->sethead();
		calendar[r]->sethead() = NULL;//head -> next = NULL;
		return 1;
	}
 return 1;
}
